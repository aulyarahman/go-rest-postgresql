...
func (db Database) GetAllItems() (*models.ItemList, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&items.ID, &items.Name, &items.Description, &items.CreatedAt)
...
func (db Database) AddIitem(item *models.Item) error {
	var id int
...
func (db Database) UpdateItem(itemId int, itemData models.Item) (models.Item, error) {
	...
	err := db.Conn.QueryRow(query, itemData.Name, itemData.Description, itemId).Scan(&item.Name, &item.Description, &item.CreatedAt)
